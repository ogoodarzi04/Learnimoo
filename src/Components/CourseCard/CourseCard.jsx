...
//
export default function CourseCard({ name, creator, description, cover, courseAverageScore, discount, price, shortName, registers, isSlide, cardStyle, isUserPanelStyle }) {
   return (
      <>
         {cover ? (
            <Card className={` overflow-hidden rounded-3xl  ${cardStyle?.[1]} dark:bg-white bg-header-color `} style={{ height: isSlide && 400 }}>
               <Link to={`/course-info/${shortName}`}>
                  <CardHeader floated={false} shadow={false} color="transparent" className="m-0 rounded-3xl cursor-pointer relative ">
                     ...
                     <img src={`http://localhost:3000/courses/covers/${cover}`} className="  bg-cover size-full" alt="ui/ux review check" />
                  </CardHeader>
                        ...
                        <>
                           ...
                           <div>
                              ...
                              <p className=" text-limon-color font-bold flex" style={{ fontSize: 16.5 }}>
                                 ...
                                 {price > 0 && <img className=" !size-8 flex !text-white" src="http://localhost:3000/courses/covers/Toman.png" alt="" />}
                              </p>
                           </div>
                        </>
                     )}
