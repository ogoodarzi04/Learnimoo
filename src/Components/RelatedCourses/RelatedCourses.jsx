...
export default function (props) {
   ...
   const fetchData = () => {
      getAllDatas(`http://learnimoo.filedl.me:3000/courses/related/${props.courseName}`, userDatas);
   };
   useEffect(() => {
      fetchData();
   }, []);
            ...
            <div className="RelatedCourse space-y-8 mt-12 bg-header-color dark:bg-white text-white dark:!text-black py-8 px-8 rounded-2xl hidden lg:block">
               ...
               {post.map((item) => (
                  <div className=" headlinecourse dark:bg-light-theme-color flex justify-between p-[8px] rounded-2xl">
                     <Link to={`http://localhost:5173/course-info/${item?.shortName}`} className=" text-white dark:!text-black">
                        <div className="Ri">
                           <div className=" flex gap-x-6">
                              <img className=" w-[96px] h-[54px] rounded-[7px]" src={`http://localhost:3000/courses/covers/${item?.cover}`} alt={item?.name} />
