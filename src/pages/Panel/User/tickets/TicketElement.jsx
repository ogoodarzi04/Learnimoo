      ...
      <div className=" Tickets ">
         <div className="bg-[rgb(40,41,61)] dark:!bg-white  md:p-8 rounded-3xl ">
            <div className="flex justify-between items-center p-4 md:pb-4 mb-6 md:mb-7 dark:!border-b-gray-200" style={{ borderBottom: "1px solid rgb(50,51,77)" }}>
               ...
               {isLeftSideElem ? (
                  <Link to={"/my-account/tickets"} className="flex items-center gap-x-1.5 text-blue-500  text-sm">
                     همه تیکت ها
               ...
               {isSlice
                  ? post.slice(0, 4).map((item) => (
                       <div className="flex items-center justify-between flex-wrap gap-y-3 p-3 dark:hover:bg-gray-100 hover:bg-[rgb(50,51,77)] rounded-xl transition-colors">
                          <Link to={`/my-account/view_ticket?id=${item._id}`} className="dark:!text-gray-700 text-white w-full sm:max-w-sm sm:truncate">
                             {item.departmentID}
                          </Link>
                  ...
                  : post.map((item) => (
                       <div className="flex  items-center justify-between flex-wrap gap-y-3 p-3 dark:hover:bg-gray-100 hover:bg-[rgb(50,51,77)] rounded-xl transition-colors">
                          <Link to={`/my-account/view_ticket?id=${item._id}`} className="dark:!text-gray-900 text-white w-full sm:max-w-sm sm:truncate">
                             #{item._id.slice(0, 5)}
                             {item.title}
                          </Link>
