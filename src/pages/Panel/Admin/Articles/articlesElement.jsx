...
export default function ArticlesElement({ isAddNewCmp }) {
   ...
   const fetchData = () => {
      getAllDatas(`${DOMAIN}articles`, userDatas);
   };
   useEffect(() => {
      fetchData;
   }, []);
   ...
   function deleteModalSubmitAction() {
      deletedata(`${DOMAIN}articles/${userId}`, userDatas);
      setIsdelete(false);
   }
   useEffect(() => {
      fetchData();
                                       ...
                                       <div className="scale-[3] absolute -right-28   rounded-full ">
                                          {item?.cover ? (
                                             <div className=" size-max flex ">
                                                {" "}
                                                <Avatar className=" scale-[0.6] " src={`http://learnimoo.filedl.me:3000/courses/covers/${item?.cover}`} alt={item.title} />
                                             </div>
                                          ) : (
