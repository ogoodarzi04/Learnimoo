...
export default function commentElement({ isAddNewCmp }) {
   ...
   const fetchData = () => {
      getAllDatas("http://localhost:3000/v1/comments", userDatas);
   };
   useEffect(() => {
      fetchData();
   }, []);
   ...
   function deleteModalSubmitAction() {
      deletedata(` http://localhost:3000/v1/comments/${userId}`, userDatas);
      setIsdelete(false);
   }
   useEffect(() => {
      fetchData();
   ...
   function banModalSubmitAction() {
      editedata(`http://localhost:3000/v1/users/ban/${userId}`, userDatas, {});
      setIsBan(false);
   ...
   function sendAnswerToUser(e) {
      ...
      //
      postdata(`http://localhost:3000/v1/comments/answer/${userId}`, newUserData, userDatas);
      ClearInputs();
   }
   const [answer, setAnswer] = useState(false);
   function rejectComment(userId) {
      editedata(`http://localhost:3000/v1/comments/reject/${userId}`, userDatas, {});
   }
   //
   function acceptComment(userId) {
      editedata(`http://localhost:3000/v1/comments/accept/${userId}`, userDatas, {});
   }
   ///
   return (
      <>
